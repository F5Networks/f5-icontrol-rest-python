...
def _validate_icruri(bigip_icr_uri):
    ...
    # TODO consider valid netlocs
    if path != '/mgmt/tm/':
        if not path.endswith('/'):
            error_message =\
            "The bigip_icr_uri must end with '/'!!  But it's: %s"\
            % path
        else:
            error_message = path
...
def _validate_suffix_collections(suffix_collections):
    ...
    if suffix_collections.endswith('/'):
        error_message =\
        "suffix_collections path element must not end with '/', but it's: %s"\
        % suffix_collections
        raise InvalidSuffixCollection(error_message)
    return True
...
class IControlRESTSession:
    ...
    def __init__(self, bigip, username, password, timeout=30, log_level=logging.DEBUG):
        ...
        self.bigip_version = None # TODO implement call to get this
        # Configure with passed parameters
        self.session.auth = (username, password)
        self.session.timeout = timeout

        # Set state as indicated by ancestral code.
        self.session.verify = False # TODO: make TOFU
